    ...
    pages: [
      {
        ...
        howItWorks: [
          "Deck header shows card count vs plan cap, a Free plan, Paid plan, or Complimentary badge (hover Paid plan or Complimentary for your tier and cards-per-deck limit; Complimentary means grant access, not a Stripe subscription), and last updated date.",
  ...
  },
  {
    ...
    pages: [
      ...
      },
      {
        ...
        doNots: [
          "Do not expect to assign AI Essay to members from Team Admin at this time.",
      ...
      },
      {
        ...
        howItWorks: [
          ...
          "Assigned members join with the lobby code (Join with code); the host assigns battle teams, then starts.",
        ...
        requirements: [
          ...
          "Explicit Live Classroom team assignment for members and team admins (owner always has access).",
          "Host permission: owner, assigned team admin, or assigned member with Can host.",
        ],
        doNots: [
          "Not available on Free, Pro, Pro Plus Individual, or Education Plus Individual.",
          "Do not expect workspace membership alone to unlock Live Classroom — assign members in Settings.",
      ...
      },
      {
        ...
        requirements: [
          ...
          "Question counts must be at least 1 and at most the eligible card total (front and back filled) before Generate or Publish.",
          "Publish requires AI content where true/false or fill-in-the-blank counts require it.",
        ],
        doNots: [
          "Do not change schedule or Exam Mode mid-session without warning members.",
          "Do not click Generate or Publish before saving format checkboxes — save deck or workspace formats first.",
          "Do not generate or publish until question counts are valid (1 through the deck total shown in the UI).",
          "Saving new format settings clears prior counts and published assignments — re-enter counts and publish after saving if needed.",
        ],
      },
      {
        id: "ws-history",
  ...
  },
  {
    ...
    pages: [
      ...
      },
      {
        ...
        howItWorks: [
          "AI Lesson Builder (/teacher/lesson-builder) — multi-day lesson plans with objectives, activities, and vocabulary; Jamaica NSC guidelines (including 5E class timelines) apply only when Learning Standard is confirmed Jamaica-linked. Website URL, Plain text, Word, and PowerPoint can be added as reference material — use Add URL before Generate, or paste page text if a site cannot be fetched. Existing deck lists workspace decks you can use (including assigned team decks) that do not already have a linked lesson plan. In edit mode, leaving via Back, sidebar/tabs, other in-app links, or browser Back with unsaved Input/Preview changes (or intake vs preview mismatches) prompts you to stay, generate a new plan (AI runs only on Generate), or keep the current preview and auto-save both intake and preview — creators overwrite their plan only while the deck is unassigned; once the deck is assigned to any member, the linked original stays frozen and creator edits save as a personal copy. Assignees never overwrite an assigned original (personal copy, with skip when already similar). Assignees editing a creator-linked assigned plan are prompted before Generate (and from the leave-dialog Generate action) to either run a completely new AI generation or create their own lesson plan from the linked plan without a new AI call (original stays unchanged; personal copy is created or updated, with content restructured to match changed intake details).",
          "AI Quiz/Test Generator (/teacher/quizzes) — classroom assessments with review before saving to a deck. Optionally Include reading passage: set Number of passages, Questions per passage (default 5, max 10), Passage type (including Auto, Scenario, Story, Dialogue, Case Study, Investigation, News Article, Journal Entry, Letter, Report, Workplace Situation, Experiment, Observation, Historical Narrative, Interview, Debate), Passage style (including Auto, Realistic, Realistic Workplace, Academic, Conversational, Narrative, Formal, Problem Solving, Investigation, Historical, Informational), Reading level (Below/On/Above Grade; default On Grade), Question types (Multiple Choice on by default; also Critical Thinking, Scenario-Based, Practical/Application — at least one required), and optional toggles for vocabulary, teacher notes, answer explanations, local/cultural context, and avoiding previous passages. A selected Lesson Plan automatically supplies curriculum context (standards, objectives, materials, activities, vocabulary as support, references); editing Subject/Grade/Topic after selection overrides the plan for generation. Generation is curriculum-driven and sequential (one passage at a time) across all subjects with server-side diversity validation/replacement: unique educational situations that teach the lesson — not one passage per vocabulary word. Each question becomes a card with the passage on the front, the correct answer on the back, and three wrong answers for quiz mode. For math and problem-solving answers on regular (non-passage) cards, the card back can include a Step 1: … Answer: … breakdown so Standard Review study mode shows the worked solution; Quiz mode still displays only the final Answer: value beside short wrong answers. Regular quiz cards + all passage questions must stay within the deck card limit. When a multi-day saved lesson plan is selected, AI Generate opens a day-scope dialog: All Days (full plan) or a single day (Day 1 (Monday) / Day 2 … when weekday labels exist), each with a short caption from that day’s daily focus or vocabulary when available; generation uses only the chosen scope’s vocab/focus/outline. Single-day plans skip the dialog. In a team workspace, assignees can pick original lesson plans linked to decks assigned to them (labeled title · grade · creator, with (Owner)/(Team Admin) when applicable; personal saves use title · grade) in addition to their own saved plans; generation uses the plan id with the workspace team id. Your Personal Decks / Your Decks lists saved quiz decks in a left-to-right slider with Search & filters (name, day label, or description) and Open Deck on each card.",
      ...
      },
      {
        ...
        howItWorks: [
          ...
          "If the linked source deck was deleted (including after you left Education and deleted the deck on a non-Education plan), the saved lesson plan can remain with a Deck deleted badge; Edit and Create Quiz are unavailable because the deck link no longer works.",
        ...
        doNots: [
          ...
          "Do not expect Edit or Create Quiz on a Deck deleted plan — recreate a deck link only by generating a new plan against a live deck.",
