...
const ALL_ARTICLES: DocArticle[] = [
    ...
    [
      ...
      },
      {
        ...
        bullets: [
          ...
          "Team Admins assign the same catalog keys from /dashboard/team-admin/add-ons.",
  ...
  a(
    "addon-plans",
    "Addon Plans — In-Depth Guide",
    "Edit add-on names, prices, and copy at /admin/addon-plans. Save syncs the matching Stripe product.",
    [
      {
        id: "editor",
        title: "Editor fields",
      ...
      },
      {
        ...
        title: "Stripe sync",
        bullets: [
          "Products are matched by metadata.flipvise_addon_key or the env monthly Price's product.",
          "Stripe cannot change a Price amount — a new Price is created when the catalog amount changes.",
          "Checkout uses catalog-aligned Prices on the same product even if Render env still points at an older Price id.",
          "Set tax code txcd_10103000 (SaaS — personal use) so Managed Payments is Eligible instead of Needs info.",
        ],
      },
    ],
  ),
    ...
    [
      {
        ...
        bullets: [
          ...
          "Checkout sessions omit payment_method_types. Extra methods such as Cash App Pay, Amazon Pay, and Klarna are excluded. Enable Cards, Apple Pay, Google Pay, Link, and ACH Direct Debit in the live Stripe Dashboard (Settings → Payment methods). Register the public app domain under Payment method domains so wallets appear.",
        ],
      },
      {
        id: "promos",
