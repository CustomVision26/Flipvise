    ...
    [
      ...
      },
      {
        ...
        bullets: [
          ...
          "Reference material: Website URL, Plain text, Word, or PowerPoint. Add URL before Generate when a website is still in the field. If a site cannot be fetched, paste the page text with Plain text.",
      ...
      },
      {
        ...
        bullets: [
          "AI Quiz/Test Generator (/teacher/quizzes) — review AI cards before saving to a deck. Set Regular quiz cards, optionally Include reading passage with Number of passages, Questions per passage (default 5, max 10), Passage type (Auto/Scenario/Story/Dialogue/Case Study/Investigation/News Article/Journal Entry/Letter/Report/Workplace Situation/Experiment/Observation/Historical Narrative/Interview/Debate), Passage style (Auto/Realistic/Realistic Workplace/Academic/Conversational/Narrative/Formal/Problem Solving/Investigation/Historical/Informational), Reading level (Below/On/Above Grade; default On Grade), Question types (Multiple Choice on by default; also Critical Thinking, Scenario-Based, Practical/Application — require at least one). Passages generate sequentially with diversity checks against prior accepted scenarios. Optional toggles (include vocabulary naturally, teacher notes, answer explanations, local/cultural context, avoid previous passages). A selected Lesson Plan is the curriculum source (standards, objectives, materials, activities, vocabulary as support, references) — teachers do not re-enter those fields; editing Subject/Grade/Topic after selection overrides the plan for generation. Passages are curriculum-driven unique educational situations for any subject with server-side diversity checks/repair; vocabulary supports the lesson and must not drive one passage per term. Card layout unchanged: FRONT = passage + related question, BACK = correct answer, plus three wrong answers for quiz mode. For math and problem-solving questions on regular (non-passage) cards, AI can put a Step 1: … Answer: … workout on the card back for Standard Review study mode, while Quiz mode shows only the final Answer: value with short wrong answers. With a multi-day saved lesson plan selected, AI Generate asks whether to use All Days (full multi-day plan) or a specific day (Day 1 (Monday), Day 2 (Wednesday), etc.); each day option can show a muted caption from that day’s daily focus or vocabulary when present. Generation uses only that day’s vocab/focus/outline (or the whole plan for All Days). Single-day plans skip the dialog. When saving from a lesson plan: All Days adds cards to `{Deck} LP All Days` if that deck already exists, otherwise creates it (never the linked main lesson deck or a Day N deck); a specific day adds cards to `{Deck} LP Day N` if that deck already exists, otherwise creates it. Decks without a linked lesson-plan source still create a new short-named deck. Team members see assigned-deck original lesson plans in the Lesson plan dropdown (alongside personal saves), labeled as title · grade · creator name with (Owner)/(Team Admin) when applicable; personal saves use title · grade. View saved PDF works when the original has a PDF. Your Personal Decks / Your Decks shows saved quiz decks in a left-to-right card slider with Search & filters (name, day label, description; Deck A–Z / Z–A) and Open Deck on each card.",
